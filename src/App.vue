  ...
  methods: {
    ...
    async fetchProdutos() {
      const response = await axios.get(
        "https://run.mocky.io/v3/7c98cb8f-963f-4f5a-b80b-2c8a55003aa7"
      );
      this.produtos = response.data;
    },

    abrirModal(id) {
      this.produto = this.produtos.find(e => e.id === id);
      window.scrollTo({
        top: 0,
      });
    },
...
.carrinho_fechar {
  ...
  font-size: 1rem;
  cursor: pointer;
  box-shadow: 0px 3px 4px rgba(0, 0, 0, 0.3), 0px 4px 10px rgba(0, 0, 0, 0.2);
}
